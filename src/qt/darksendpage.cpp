...
void DarksendPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeKonjungateAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeKonjungateAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        //strAnonymizeKonjungateAmount = strAnonymizeKonjungateAmount.remove(strAnonymizeKonjungateAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeKonjungateAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeKonjungateAmount*COIN) nMaxToAnonymize = nAnonymizeKonjungateAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeKonjungateAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeKonjungateAmount));
        //strAnonymizeKonjungateAmount = strAnonymizeKonjungateAmount.remove(strAnonymizeKonjungateAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeKonjungateAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
    ...
    QString strDenomBalance = QString().setNum(denomBalance, 'g', 4) + " KONJ";
