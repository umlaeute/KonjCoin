...
//
// Bootup the masternode, look for a 600000 KONJ input and register on the network
//
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if (status != MASTERNODE_REMOTELY_ENABLED)
        {
            ...
            if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
                ...
                /* rewards are not supported in Konjungate.conf */
...
{
    CKonjungatecoinAddress address(collateralAddress);
