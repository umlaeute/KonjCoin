...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <Konjungateaddress> <amount>\n"
            "Konjungateaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <Konjungateaddress> <amount>\n"
            "Konjungateaddress, denominate, or auto (AutoDenominate)"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (strCommand == "genkey")
    {
        ...
        return CKonjungatecoinSecret(secret).ToString();
    }
