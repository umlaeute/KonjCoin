...
int SecureMsgAddWalletAddresses()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(CTxDestination, std::string)& entry, pwalletMain->mapAddressBook)
    {
        ...
        CKonjungatecoinAddress coinAddress(entry.first);
...

{
    ...
    {
    std::string sReason;
        // - konjungate public keys from coinstake txns
        if (tx.IsCoinStake())
        {
...
int SecureMsgScanMessage(uint8_t *pHeader, uint8_t *pPayload, uint32_t nPayload, bool reportToGui)
{
    ...
    */
    ...
    for (std::vector<SecMsgAddress>::iterator it = smsgAddresses.begin(); it != smsgAddresses.end(); ++it)
    {
        ...
        CKonjungatecoinAddress coinAddress(it->sAddress);
...
int SecureMsgAddAddress(std::string& address, std::string& publicKey)
{
    ...
    */
    ...
    CKonjungatecoinAddress coinAddress(address);
    ...
    CKonjungatecoinAddress addressT(keyIDT);
...
int SecureMsgEncrypt(SecureMessage &smsg, const std::string &addressFrom, const std::string &addressTo, const std::string &message)
{
    ...
    */
    ...
    bool fSendAnonymous;
    CKonjungatecoinAddress coinAddrFrom;
    CKeyID ckidFrom;
    CKey keyFrom;
    ...
    CKonjungatecoinAddress coinAddrDest;
    CKeyID ckidDest;
    ...
    } else
    {
        ...
        vchPayload[0] = (static_cast<CKonjungatecoinAddress_B*>(&coinAddrFrom))->getVersion(); // vchPayload[0] = coinAddrDest.nVersion;
...
int SecureMsgSend(std::string &addressFrom, std::string &addressTo, std::string &message, std::string &sError)
{
    ...
    */
    ...
    CKonjungatecoinAddress coinAddrOutbox;
    ...
    BOOST_FOREACH(const PAIRTYPE(CTxDestination, std::string)& entry, pwalletMain->mapAddressBook)
    {
        ...
        const CKonjungatecoinAddress& address = entry.first;
...
int SecureMsgDecrypt(bool fTestOnly, std::string &address, uint8_t *pHeader, uint8_t *pPayload, uint32_t nPayload, MessageData &msg)
{
    ...
    */
    ...
    } else
    {
        ...
        CKonjungatecoinAddress coinAddrFromSig;
