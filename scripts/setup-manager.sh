...
purgeOldInstallation() {
    ...
    test -d ~/KonjungateBackups && echo "Backups folder exists, skipping directory creation..." || mkdir ~/KonjungateBackups
    iteration=0
    ...
    mv $CONFIG_FOLDER/Konjungate.conf $BACKUP_FOLDER/$foldername
...
function important_information() {
    ...
    echo -e "${BLUE}================================================================================================================================"
    echo -e "${CYAN}Follow twitter to stay updated.  https://twitter.com/KonjungateCoin${NC}"
    echo -e "${BLUE}================================================================================================================================${NC}"
    echo -e "${CYAN}Ensure Node is fully SYNCED with BLOCKCHAIN before starting your Node :).${NC}"
    echo -e "${BLUE}================================================================================================================================${NC}"
    echo -e "${GREEN}Usage Commands.${NC}"
    echo -e "${GREEN}konjungated masternode status${NC}"
    echo -e "${GREEN}konjungated getinfo${NC}"
    echo -e "${BLUE}================================================================================================================================${NC}"
    echo -e "${NC}"
}
...
function setup_node() {
  get_ip
  update_config
  systemctl restart konjungate
}

function install_konjungate() {
    echo "You chose to install the Konjungate Node"
    echo "Checking for Konjungate installation"
    if [ -e /usr/bin/konjungated ] || [ -e /usr/local/bin/konjungated ]; then
        purgeOldInstallation
    else
...
function compile_linux_daemon() {
    echo "You chose to compile the Konjungate CLI Daemon/Wallet"
    checks
    prepare_system
    if [ ! -e ../Konjungate.pro ] ; then
...
function setup_masternode() {
    ...
    if [ -e /usr/bin/konjungated ] || [ -e /usr/local/bin/konjungated ]; then
        read -p "There is already an installation of Konjungate Coin. Did you want to use the currently installed software, or install the latest software? Y/n:" yn
        case $yn in
            [Yy]* ) install_konjungate; setup_node; important_information;;
...
function install_dependencies_only() {
    echo "You chose to install Konjungate dependencies only"
    prepare_system
}
...
function backup_node_data() {
    ...
    today="$( date +"%Y%m%d" )"
    #Create a backups folder inside users home directory
    test -d ~/KonjungateBackups && echo "Backups folder exists" || mkdir ~/KonjungateBackups
    iteration=0
    while test -d "$BACKUP_FOLDER/$today$suffix"; do
        (( ++iteration ))
        suffix="$( printf -- '-%02d' "$iteration" )"
    ...
    cp $CONFIG_FOLDER/Konjungate.conf $BACKUP_FOLDER/$foldername
    cp $CONFIG_FOLDER/wallet.dat $BACKUP_FOLDER/$foldername
}

function uninstall() {
    read -p "You chose to uninstall Konjungate, would you like to continue? y/n:" yn
    case $yn in
...
echo "========================================================="
echo "[1] - Install Konjungate node (will uninstall/upgrade existing installation)"
echo "[2] - Compile only (will compile linux daemon, linux GUI, or windows GUI - no install)"
echo "[3] - Prepare masternode (will install Konjungate Node if needed)"
echo "[4] - Install dependencies only"
echo "[5] - Backup Konjungate wallet and settings"
echo "[6] - Uninstall Konjungate"

read choice1

case $choice1 in
    "1") install_konjungate;;
    "2") compile_only;;
    "3") setup_masternode;;
